...
class LentaServicePlug {
  ...
  Future<ApiTegList> GetTegs() =>
      Future.delayed(Duration(milliseconds: 100), () {
        return ApiTegList.fromApi(AllTegs.entries
            .map((e) => ApiTeg.fromApi(e.key, e.value))
            .toList());
      });
  ...
  Future<ApiPostList> GetPosts(GetPostRequestBody body) =>
      Future.delayed(Duration(milliseconds: 100), () {
        var posts = AllPosts;
        if (body.time != null) {
          posts = AllPosts.where((e) => e.timeofcreation.isAfter(body.time))
              .toList();
        }
        List<ApiPost> res = [];
        if (body.tegs.length > 0) {
          for (var p in posts) {
            if (p.tegs.any(
                (element) => body.tegs.map((e) => e.name).contains(element))) {
              res.add(p);
            }
          }
        } else {
        ...
        print(res.length);
        return ApiPostList.fromApi(
            posts: res.length > postpageCap
                ? res.take(postpageCap).toList()
                : res);
      });
